...
const getStreamHead = `-- name: GetStreamHead :one
...
func (q *Queries) GetStreamHead(ctx context.Context, userID uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, getStreamHead, userID)
	var head int64
	err := row.Scan(&head)
	return head, err
}
