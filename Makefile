...
# the passwords are generated by kind-up the first time it brings a cluster up,
# so a value in this file would be a value that is wrong for every cluster but
# one. What is rewritten is only where the database answers — inside it is a
# service, and from here it is a port kind published.
